...
const char kMqttCommands[] PROGMEM = "|" // No prefix
    ...
    D_CMND_TLSKEY "|"
#endif
    D_CMND_MQTTHOST "|" D_CMND_MQTTPORT "|" D_CMND_MQTTRETRY "|" D_CMND_STATETEXT "|" D_CMND_MQTTCLIENT "|" D_CMND_FULLTOPIC "|" D_CMND_PREFIX "|" D_CMND_GROUPTOPIC "|" D_CMND_TOPIC "|" D_CMND_PUBLISH "|" D_CMND_MQTTLOG "|" D_CMND_BUTTONTOPIC "|" D_CMND_SWITCHTOPIC "|" D_CMND_BUTTONRETAIN "|" D_CMND_SWITCHRETAIN "|" D_CMND_POWERRETAIN "|" D_CMND_SENSORRETAIN;
...
#ifdef MDNS_HOSTNAME
    for (i = n; i > 0; i--)
    { // Search from last to first and use first if not found
...
#else
  tlsClient->setTrustAnchor(&RaspBee_TA);
#endif // USE_MQTT_AWS_IOT
...
void MqttPublishPowerState(uint32_t device)
{
  ...
  if (IsModuleIfan() && (device > 1))
  {
    ...
    { // 4 occurs when fanspeed is 3 and RC button 2 is pressed
#ifdef USE_DOMOTICZ
...
void MqttPublishAllPowerState(void)
{
  for (uint32_t i = 1; i <= devices_present; i++)
  {
    ...
    {
      break;
    }  // Report status of light relay only
#endif // USE_SONOFF_IFAN
  }
}
...
#ifdef USE_DISCOVERY
#ifdef MQTT_HOST_DISCOVERY
        if (!strlen(SettingsText(SET_MQTT_HOST)) && !Wifi.mdns_begun)
        {
          return;
        }
#endif // MQTT_HOST_DISCOVERY
...
void CmndFullTopic(void)
{
  if (XdrvMailbox.data_len > 0)
  {
    ...
    if (!strcmp(XdrvMailbox.data, mqtt_client))
    {
      SetShortcutDefault();
    }
    char stemp1[TOPSZ];
...
void CmndButtonTopic(void)
{
  if (!XdrvMailbox.grpflg && (XdrvMailbox.data_len > 0))
  {
    ...
    {
    case SC_CLEAR:
      SettingsUpdateText(SET_MQTT_BUTTON_TOPIC, "");
      break;
    case SC_DEFAULT:
      SettingsUpdateText(SET_MQTT_BUTTON_TOPIC, mqtt_topic);
      break;
    case SC_USER:
      SettingsUpdateText(SET_MQTT_BUTTON_TOPIC, MQTT_BUTTON_TOPIC);
      break;
    default:
      SettingsUpdateText(SET_MQTT_BUTTON_TOPIC, XdrvMailbox.data);
    }
  }
...
void CmndSwitchTopic(void)
{
  if (!XdrvMailbox.grpflg && (XdrvMailbox.data_len > 0))
  {
    ...
    case SC_CLEAR:
      ...
      break;
    case SC_DEFAULT:
      SettingsUpdateText(SET_MQTT_SWITCH_TOPIC, mqtt_topic);
      break;
    case SC_USER:
      SettingsUpdateText(SET_MQTT_SWITCH_TOPIC, MQTT_SWITCH_TOPIC);
      break;
    default:
      SettingsUpdateText(SET_MQTT_SWITCH_TOPIC, XdrvMailbox.data);
    }
  }
